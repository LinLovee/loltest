...
function ChatWindow({ currentUser, selectedUser, socket, onBack, onlineUsers }) {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (message) => {
      if (selectedUser && message.senderId === selectedUser.id) {
        // Перезагружаем все сообщения чтобы получить ID из БД
        loadMessages()
      }
    }
    ...
    try {
      ...
      if (videoPreviewRef.current) {
        ...
        try {
          ...
          // На некоторых устройствах нужен пользовательский жест
        }
      }
  ...
  const switchCamera = async () => {
    ...
    try {
      ...
      // Перезапускаем recorder с новым стримом
      ...
      {isRecordingVideo && (
        <div className="video-note-recording">
          <div className="video-note-preview">
            ...
            <button onClick={switchCamera} className="switch-camera-btn" title="Переключить камеру">
              🔄
            </button>
          </div>
