...
router.post('/upload', authenticateToken, upload.single('file'), async (req, res) => {
  ...
  try {
    // Получаем базовый URL backend из переменной окружения или из запроса
    const backendUrl = process.env.BACKEND_URL || `${req.protocol}://${req.get('host')}`;
    const fileUrl = `${backendUrl}/uploads/${req.file.filename}`;
    
    console.log('💾 Saving to DB:', { fileUrl, messageType });
...
router.put('/edit/:messageId', authenticateToken, async (req, res) => {
  ...
  console.log('✏️ Edit message request:', { messageId, userId, text: text?.substring(0, 20) });
  ...
  try {
    ...
    if (result.rows.length === 0) {
      console.log('❌ Message not found or cannot be edited');
